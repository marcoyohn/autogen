...
class EnInkRecognitionStructureAgent(autogen.AssistantAgent):
    def __init__(self, message_processor=None, context=None, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.message_processor = message_processor        
        self.context = context
        self.register_reply(Agent, EnInkRecognitionStructureAgent._en_generate_reply)
        self.url = os.environ["HTTP_API_URL_CVTE_INK_STRUCTURE"]
    ...
    def _en_generate_reply(
        ...
        request_strokers = []
        for stroke_id, points in input_strokers.items():
            points_str = ""
            ...
            for i, point in enumerate(points):
                if i == points_last_index:
                    ...
                    points_str = f"{points_str}{point},"
            request_strokers.append({"stroke_id": str(stroke_id), "points": points_str})
        ...
        if result["statusCode"] != 0:
            raise RuntimeError('手写批注结构化识别失败')    
        
        blocks = []
        for index, line in enumerate(result["data"]["result"][0]):
            items = []
            for item_index, item in enumerate(line["children"]):
                items.append({"item_index": item_index + 1, "type": item["types"], "strokes_id_list":item["strokes_id_list"]})
            
            blocks.append({"block_index": index + 1, "items": items})
