...
class FastAPIUser(BaseUser):
    ...
    def __init__(self, user_name: str, nick_name: str, real_name: str, user_id: any):
...
class FastAPIAuthBackend(AuthenticationBackend):
    ...
    def __init__(self, verify_func: Callable[[Dict], Tuple[List[str], BaseUser]], excluded_urls: List[str] = None):
        """ Auth Backend constructor. Part of an AuthenticationMiddleware as backend.

        Args:
            verify_func (callable): A function handle that returns a list of scopes and a BaseUser
    ...
    async def authenticate(self, conn: HTTPConnection) -> Tuple[AuthCredentials, BaseUser]:
        ...
        try:
            if inspect.iscoroutinefunction(self.verify_func):
                scopes, user = await self.verify_func(conn)
            else:
                scopes, user = self.verify_func(conn)
...
def AuthMiddleware(
        app: FastAPI,
        verify_func: Callable[[str], Tuple[List[str], BaseUser]],
    ...
    Args:
        ...
        verify_func (Callable[[str], Tuple[List[str], BaseUser]]): A function handle that returns a list of scopes and a BaseUser
    ...
    Examples:
        ...
        ```
    """
    return AuthenticationMiddleware(app, backend=FastAPIAuthBackend(verify_func=verify_func, excluded_urls=excluded_urls), on_error=auth_error_handler)